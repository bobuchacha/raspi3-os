#define ROS_BUILDING_JPEG_DLL 1
...
namespace {
    ...
    /*
     * Yield periodically during JPEG decode and scale loops.
     *
     * Wallpaper and photo-sized images spend most of their time in nested MCU
     * and scaling loops after the bytes are already in memory. A bounded pause
     * every few rows keeps the cooperative userspace scheduler moving without
     * materially changing decode throughput.
     *
     * @param iteration Zero-based loop counter that just completed.
     * @param stride Number of iterations between cooperative yields.
     * @param more_work_expected True when more iterations remain.
     * @return Nothing.
     */
    inline void jpeg_cooperative_work_yield(U32 iteration, U32 stride, bool more_work_expected) {
    ...
    static Status jpeg_decode_rgb_image(JpegDecoder* decoder, U8** rgb_out) {
        ...
        for (U32 block_y = 0U; block_y < blocks_y; ++block_y) {
            ...
            jpeg_cooperative_work_yield(block_y, 2U, block_y + 1U < blocks_y);
        }
    ...
    static void jpeg_render_rgb_to_target(
        ...
        for (U32 row = 0U; row < target->height; ++row) {
            ...
            jpeg_cooperative_work_yield(row, 16U, row + 1U < target->height);
        }
    }
...
/*
 * DLL entrypoint used by the loader to attach this codec module to a process.
 *
 * The JPEG helper keeps no process-local mutable state, so attach and detach
 * notifications only need to return success so the loader can finish binding
 * the dedicated jpeg.dll image.
 *
 ...
 */
extern "C" int jpeg_entry(void* image_base, U32 reason) {
    (void)image_base;
    (void)reason;
...
DLL_EXPORT(jpeg_render_to_surface);
