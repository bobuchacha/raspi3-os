...
static void* png_allocate(Size bytes) {
    ...
    return user_shared_heap_malloc((size_t)bytes);
}
...
static long png_huffman_build(PngHuffmanTree* tree, const U8* lengths, U32 symbol_count) {
    ...
    for (symbol = 0U; symbol < symbol_count; ++symbol) {
        length = lengths[symbol];
        if (length != 0U) {
            status = png_huffman_insert(tree, next_code[length], length, (I32)symbol);
            if (status != StatusOK) {
                return status;
            }
            next_code[length] += 1U;
...
/*
 * Decode and draw one PNG file into the caller canvas.
 *
 * @param path PNG file path.
 * @param memory Destination canvas description.
 * @return Zero on success, or a negative status code on failure.
 */
static long png_render_file_to_memory(const char* path, RosPngMemory* memory) {
    U8* file_bytes;
    Size file_size;
    U8* idat_bytes;
    Size idat_size;
    ...
    if ((path == 0) || (memory == 0) || (memory->buffer == 0) || (memory->width == 0U) || (memory->height == 0U)) {
        return StatusInvalidArgument;
    }
    ...
    status = png_parse_png_file(file_bytes, file_size, &header, &idat_bytes, &idat_size);
...
long PngGetInfo(const char* path, U32* width, U32* height) {
    ...
    if (!png_has_signature(header_bytes, PNG_SIGNATURE_SIZE)) {
        return StatusNotSupported;
    }

    chunk_length = png_read_be_u32(header_bytes + PNG_SIGNATURE_SIZE);
    chunk_type = png_read_be_u32(header_bytes + PNG_SIGNATURE_SIZE + 4U);
    if ((chunk_length != 13U) || (chunk_type != PNG_CHUNK_IHDR)) {
        return StatusNotSupported;
    }

    status = png_parse_ihdr(header_bytes + PNG_SIGNATURE_SIZE + 8U, 13U, &header);
    if (status != StatusOK) {
        return status;
    }
...
long PngRenderFileToMemory(const char* path, RosPngMemory* memory) {
    return png_render_file_to_memory(path, memory);
}
