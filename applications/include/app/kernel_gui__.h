...
#define ROS_KERNEL_GUI_CONTROL_WINDOW_SURFACE_CREATE 32UL
#define ROS_KERNEL_GUI_CONTROL_WINDOW_SURFACE_DESTROY 33UL
#define ROS_KERNEL_GUI_CONTROL_WINDOW_SURFACE_ACQUIRE 34UL
#define ROS_KERNEL_GUI_CONTROL_WINDOW_SURFACE_RELEASE 35UL

#define ROS_KERNEL_GUI_PROTOCOL 0x4B475549UL
#define ROS_KERNEL_GUI_KIND_SHARED_INPUT_READY 1UL
...
#define ROS_KERNEL_GUI_WINDOW_SURFACE_MAX_SLOTS 2048U
...
/*
 * One shared window-surface view mapped into either GWES or the owning client.
 *
 * GWES creates the surface once when it accepts `CreateWindow`. The client-side
 * GDI helper later acquires the same surface by `HWND`, receives a stable user
 * pointer, draws locally through the active backend, and notifies GWES about
 * dirty regions through the lightweight window IPC channel.
 */
typedef struct GuiWindowSurfaceViewStruct {
    uint32_t version;
    uint32_t flags;
    uint64_t hwnd;
    int64_t owner_pid;
