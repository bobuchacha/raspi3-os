...
#define SYSTEMTEST_EXPECTED_PROFILE "dll-loader-smoke-v1"
...
DLL_IMPORT_FUNCTION(SYSTEMTEST_DLL_NAME, "samplemath_invocation_count", systemtest_invocation_count_fn, g_systemtest_invocation_count);
DLL_IMPORT_FUNCTION(SYSTEMTEST_DLL_NAME, "samplemath_profile", systemtest_profile_fn, g_systemtest_profile);
DECLARE(long, add3, (long first_number, long second_number, long third_number), FROM, SYSTEMTEST_DLL_NAME, "samplemath_add3");
 ...
 * `XBGR8888` transport format on some boards. The pixel verifier must mirror
 * that translation so on-screen checks validate the real stored bytes.
 *
...
static void systemtest_console_verify_gwes(void) {
    long pid = findUserTaskPidByName(ROS_WINDOW_SERVER_NAME);
