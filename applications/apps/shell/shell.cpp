...
/*
 * shell_async_spawn_callback
 *
 * Report one async background launch completion through the shell console.
 *
 * Explorer uses the same callback-capable runtime path, so this callback gives
 * the headless shell a direct way to show whether async launch requests are
 * succeeding, failing immediately, or timing out without a completion.
 *
 * @param path Executable path that was queued.
 * @param pid Child pid on success, or a negative value on failure.
 * @param status Final kernel status reported for the async request.
 * @param name Visible task name supplied to the loader.
 * @param args Launch arguments supplied to the loader.
 * @param context Unused callback context.
 * @return Nothing.
 */
static void shell_async_spawn_callback(const char* path, long pid, long status, const char* name, const char* args, void* context);
...
static void shell_async_spawn_callback(const char* path, long pid, long status, const char* name, const char* args, void* context) {
    ...
    cursor = appendText(cursor, "async launch complete: status=");
    cursor = shell_append_long(cursor, status);
    cursor = appendText(cursor, " pid=");
    cursor = shell_append_long(cursor, pid);
    cursor = appendText(cursor, " path=");
    cursor = appendText(cursor, (path != NULL) ? path : "<null>");
    *cursor = '\0';
    writeLine(line);
}
...
/*
 * shell_spawn_program_async
 *
 * Queue one background launch through the callback-capable async runtime path.
 *
 * The shell keeps its normal path validation and name derivation logic so the
 * resulting behavior differs from `start` only in the underlying spawn
 * primitive. That makes this command a clean probe for Explorer's launch path.
 *
 * @param path Fully resolved executable path.
 * @param name_arg Optional caller-supplied task name.
 * @param args_arg Optional launch arguments.
 * @return Non-negative async request id on success, or a negative status code on failure.
 */
static long shell_spawn_program_async(const char* path, const char* name_arg, const char* args_arg) {
    char name[32];
    char buffer[256];
    long request_id;
    ...
    request_id = spawnTaskAsyncCallback(path, name, args_arg, shell_async_spawn_callback, NULL);
    if (request_id < 0) {
        return request_id;
    }

    shell_print_heading("Queued async task");
    {
        char* cursor = buffer;

        cursor = shell_append_long(cursor, request_id);
        *cursor = '\0';
    }
    shell_print_key_value("request", buffer);
    shell_print_key_value("name", name);
    shell_print_key_value("path", path);
    if (args_arg && args_arg[0] != '\0') {
        shell_print_key_value("args", args_arg);
...
/*
 * shell_try_spawn_candidate_async
 *
 * Validate one candidate path and queue it through the async runtime path.
 *
 * @param path Fully resolved executable path candidate.
 * @param name_arg Optional caller-supplied task name.
 * @param args_arg Optional launch arguments.
 * @return One when the candidate launched, zero when it does not exist, or a negative status code on failure.
 */
static int shell_try_spawn_candidate_async(const char* path, const char* name_arg, const char* args_arg) {
    long request_id;
    ...
    request_id = shell_spawn_program_async(path, name_arg, args_arg);
    if (request_id < 0) {
        return (int)request_id;
    }

    shell_write_line("async task request is queued");
    return 1;
}
...
/*
 * shell_try_spawn_target_async
 *
 * Resolve one command or path using the shell search rules, then queue it
 * through the async runtime path used by Explorer.
 *
 * @param state Current shell state containing the working directory and PATH.
 * @param input Command or path supplied by the user.
 * @param name_arg Optional caller-supplied task name.
 * @param args_arg Optional launch arguments.
 * @return Zero on success, one when no executable matched, or a negative status code on failure.
 */
static int shell_try_spawn_target_async(ShellState* state, const char* input, const char* name_arg, const char* args_arg) {
    char candidate[SHELL_PATH_MAX];
    char with_ext[SHELL_PATH_MAX];
    int result;
    int index;
    ...
    if (shell_path_has_drive_prefix(input) || shell_path_contains_separator(input) || input[0] == '/' || input[0] == '\\') {
        ...
        if (!shell_path_has_extension(candidate) && shell_strlen(candidate) + 4 < sizeof(with_ext)) {
            ...
            with_ext[sizeof(with_ext) - 1] = '\0';
            shell_strcat(with_ext, ".exe");
            result = shell_try_spawn_candidate_async(with_ext, name_arg, args_arg);
            if (result > 0) {
                return 0;
            }
            if (result < 0) {
        ...
        result = shell_try_spawn_candidate_async(candidate, name_arg, args_arg);
        if (result > 0) {
            return 0;
        }
        return result < 0 ? result : 1;
...
static void shell_print_help(void) {
    ...
    shell_print_command_help("startasync [--name n] <cmd> [-- args]", "Queue a program through the async runtime path");
    ...
    shell_print_command_help("msgbox [error] <text>", "Show a built-in window.dll message box");
...
/*
 * shell_command_startasync
 *
 * Exercise the callback-capable async runtime launch path from the shell.
 *
 * This command intentionally mirrors `start` so a headless run can compare the
 * known-good synchronous background launch path with the same path resolution
 * and name derivation routed through async spawn.
 *
 * @param state Mutable shell state for path resolution.
 * @param full_args Command tail entered by the user.
 * @return Nothing.
 */
static void shell_command_startasync(ShellState* state, const char* full_args) {
    char buffer[SHELL_LINE_MAX];
    char args[SHELL_LAUNCH_ARGS_MAX];
    char* tokens[SHELL_TOKEN_MAX];
    const char* target_arg;
    ...
    if (!full_args || full_args[0] == '\0') {
        shell_print_usage("startasync [--name task] <command|path> [-- args]");
        return;
    }
    ...
    result = shell_try_spawn_target_async(state, target_arg, name_arg, args);
    if (result == 1) {
        shell_print_error("startasync", "program not found");
    }
    else if (result != 0) {
        shell_print_error("startasync", "failed to queue async program");
    }
}
...
/*
 * Show one built-in GUI message box from the shell process.
 *
 * This gives window.dll a real in-system caller that exercises both the basic
 * and error-notification variants without depending on explorer's launch path.
 *
 * @param full_args Optional `error` selector followed by message text.
 * @return Nothing.
 */
static void shell_command_msgbox(const char* full_args) {
    char buffer[SHELL_LINE_MAX];
    char text[SHELL_LAUNCH_ARGS_MAX];
    char* tokens[SHELL_TOKEN_MAX];
    unsigned long flags = ROS_MESSAGEBOX_FLAG_NONE;
    int count;
    int start_index = 0;
    long result;

    if (!full_args || full_args[0] == '\0') {
        shell_print_usage("msgbox [error] <text>");
        return;
    }
    ...
    if (shell_same_text_case_insensitive(tokens[0], "error")
        || shell_same_text_case_insensitive(tokens[0], "/error")
        || shell_same_text_case_insensitive(tokens[0], "-e")) {
        flags = ROS_MESSAGEBOX_FLAG_ERROR;
        start_index = 1;
    }
    ...
    shell_join_tokens(text, sizeof(text), tokens, start_index, count);
    result = (flags & ROS_MESSAGEBOX_FLAG_ERROR) != 0UL
        ? MessageBoxError("Shell Error", text)
        : MessageBox("Shell", text);
    if (result != ROS_MESSAGEBOX_RESULT_OK) {
        shell_write_line("msgbox: dialog dismissed or failed");
    }
}
...
static int shell_execute_builtin(ShellState* state, const char* command, const char* arg0, const char* full_args) {
    ...
    if (shell_same_text_case_insensitive(command, "startasync")) {
        shell_command_startasync(state, full_args);
        return 0;
    }
    ...
    if (shell_same_text_case_insensitive(command, "msgbox")) {
        shell_command_msgbox(full_args);
        return 0;
    }
 ...
 * main
 *
 * Userspace programs in this kernel now enter through the shared `_start`
 * runtime wrapper, which then calls the application-owned `main()`. The
 * restored legacy shell previously depended on the older direct-entry
 * convention, so converting the tree to C++ front-ends also normalizes the
 * shell onto the same hosted-style entrypoint as the rest of userspace.
 *
 * Keeping the shell on `main()` means the C++ runtime can run constructors and
 * destructors consistently without each application hand-rolling its own
 * startup glue.
 */
int main(void) {
    ShellState state;
