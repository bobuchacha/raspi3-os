...
static unsigned long widgetdemo_tick_count = 0UL;
...
/*
 * Refresh the demo captions so button clicks, textbox edits, timer ticks, and
 * resize notifications stay visible without relying on the serial log.
 *
 * @return Nothing.
 */
...
static long widgetdemo_paint_dialog(HWND hwnd) {
    ...
    snprintf(uptime_line, sizeof(uptime_line), "Dialog WM_PAINT tick=%lu", widgetdemo_tick_count);
    (void)WidgetDrawText(&context, 24UL, footer_y + 4UL, uptime_line, 0x00303E4FUL);
