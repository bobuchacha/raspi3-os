...
extern "C" {
    ...
    /*
     * Compute how long the window/render thread may idle before re-checking work.
     *
     * The current runtime still lacks a wait object that can be signaled by both
     * the explicit window queue and the compositor damage path, so the scheduler
     * clamps idle sleeps to a short slice while still honoring the next due timer.
     *
     * @param now Current uptime in milliseconds.
     * @return Milliseconds the window thread may sleep before polling again.
     */
