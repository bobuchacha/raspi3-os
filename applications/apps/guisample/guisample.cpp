...
#define GUISAMPLE_TIMER_MSEC 200UL

static HWND g_guisample_main_window = 0UL;
static unsigned long g_guisample_timer_id = 0UL;
...
int main(void) {
    ...
    g_guisample_timer_id = SetTimer(g_guisample_main_window, 0UL, GUISAMPLE_TIMER_MSEC);
    if (g_guisample_timer_id == 0UL) {
        writeLine("guisample.exe: failed to register timer");
        return 1;
    }
    ...
    if (g_guisample_main_window != 0UL && g_guisample_timer_id != 0UL) {
        (void)KillTimer(g_guisample_main_window, g_guisample_timer_id);
        g_guisample_timer_id = 0UL;
    }
    if (g_guisample_font_ready) {
