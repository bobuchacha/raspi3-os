...
static volatile unsigned int dbg_pause_active = 0;
static volatile unsigned int dbg_pause_owner_cpu = MAX_CPUS;
...
static inline void dbg_pause_barrier(void) {
    asm volatile("dmb ish" ::: "memory");
}

void dbg_pause_world(void) {
    unsigned int owner_cpu = task_cpu_index();

    dbg_pause_owner_cpu = owner_cpu;
    dbg_pause_barrier();
    dbg_pause_active = 1;
    dbg_pause_barrier();

    for (unsigned int cpu = 1; cpu < MAX_CPUS; ++cpu) {
        if (cpu == owner_cpu || !percpu_cpu_is_online(cpu)) {
            continue;
        }
        percpu_kick_cpu(cpu);
    }
...
void dbg_resume_world(void) {
    dbg_pause_barrier();
    dbg_pause_active = 0;
    dbg_pause_owner_cpu = MAX_CPUS;
    dbg_pause_barrier();

    for (unsigned int cpu = 1; cpu < MAX_CPUS; ++cpu) {
        if (!percpu_cpu_is_online(cpu)) {
            continue;
        }
        percpu_kick_cpu(cpu);
    }
...
void dbg_wait_if_paused(void) {
    unsigned int cpu = task_cpu_index();

    while (1) {
        dbg_pause_barrier();
        if (!dbg_pause_active || cpu == dbg_pause_owner_cpu) {
            return;
        }
