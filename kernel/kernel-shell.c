...
static void shell_forward_gui_key(char ch) {
    ...
    if (module_invoke("usbkey", "inject_key", (unsigned long)(unsigned char)ch, 0, &result) != 0) {
        module_invoke("fbgui", "push_key", (unsigned long)(unsigned char)ch, 0, &result);
    }
}

static void shell_putcp(void* ptr, char ch) {
    *(*((char**)ptr))++ = ch;
}
...
static void shell_show_hardware_info(void) {
    ...
    shell_print_kv("board", "%s", "Raspberry Pi 3 Model B target");
...
static void shell_print_help(void) {
    ...
    kprint("  "); shell_print_text_column("libs", SHELL_HELP_COL_WIDTH, SHELL_COLOR_LABEL); kprint(" List loaded shared libraries and export tables\r\n");
...
static void shell_print_library_local(const UserSharedLibraryInfo* infos, UInt info_count, const UserSharedLibraryLocal* local, int depth) {
    char number[32];
