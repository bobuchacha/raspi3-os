...
Bool vfs_has_root_filesystem(void) {
	return vfs_mount_table[0].fs_driver != 0;
}
...
int vfs_file_get_size(const char* filename) {
    ...
    filepath.pathbuf = (Pointer)kmalloc(4096);
	...
	if (fs_id < 0 || fs_id >= VFS_MOUNT_TABLE_MAX || !vfs_mount_table[fs_id].fs_driver) {
