							...
							struct FAT32DirEntry *dir_dest)
{
	...
	for (int i = 0;; i += 32)
	{
		...
		UByte *name_u8 = null;
		char short_name[13];
		memzero((Address)name_u16, sizeof(name_u16));
		memzero((Address)short_name, sizeof(short_name));
		fat32_get_full_name(&dir, short_name);
		...
		else
		{
			...
			strcpy((char *)name_u8, short_name);
		}

		if (strcmp(name, (char *)name_u8) == 0 || strcmp(name, short_name) == 0)
		{
...
int fat32_dir_read_ex(void *private, struct DirectoryEntry *vfs_dir_entry, unsigned int cluster, unsigned int entry)
{
	...
	memzero((Address)vfs_dir_entry, sizeof(*vfs_dir_entry));
	memzero((Address)lfn_entries, sizeof(lfn_entries));
	...
	for (int i = entry * 32;; i += 32)
	{
		...
		else
		{
			...
			memzero((Address)buff, sizeof(buff));
...
int _write_lfn(void *priv, unsigned int cluster, unsigned char *filename, struct FAT32DirEntry *dir)
{
	...
	for (int i = 0; i < lfn_entry_count; i++)
	{
		...
		for (int j = 0, _d = 0; j < 10; j++, _d++, _s++)
			lfnEntry->FLN_NameFirst5[_d] = *(((UByte *)longname) + _s);
		for (int j = 0, _d = 0; j < 12; j++, _d++, _s++)
			lfnEntry->LFN_Name6to11[_d] = *(((UByte *)longname) + _s);
		for (int j = 0, _d = 0; j < 4; j++, _d++, _s++)
			lfnEntry->LFN_NameLast2[_d] = *(((UByte *)longname) + _s);
		lfnEntry->FLN_Attr = ATTR_LONG_NAME;
		lfnEntry->LFN_SequenceNumber = (i + 1) & 0x1F; // sequence number
		if (i == lfn_entry_count - 1)
			lfnEntry->LFN_SequenceNumber |= 0x40; // mark last entry en the sequence
	...
	for (int i = lfn_entry_count; i > 0; i--)
	{
		...
		if ((ret = fat32_dir_insert_entry(priv, cluster, (struct FAT32DirEntry *)lfnEntry)) < 0)
			return ret;
	}
}
