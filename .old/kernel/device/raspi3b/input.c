 ...
 * input.c
 *
 * Board-local console input implementation.
 *
 * The queue itself is generic kernel infrastructure, but the blocking fallback
 * source is the raspi3b UART device, so the implementation belongs with the
 * rest of the board device code rather than at the kernel root.
 */
#include "input.h"
...
/* Small AArch64 spin lock used to protect the injected-key ring buffer. */
...
/* Queue one externally injected character for the console read path. */
...
/* Remove one queued injected character when one is available. */
...
/* Read console input from injected sources first, then fall back to UART. */
