...
void _mmu_map_kernel(void) {
    ...
    /*
     * QEMU virt starts the loader in RAM at 0x40080000. Keep the low 0..1GB
     * region mapped for devices and boot handoff buffers, and add the 1..2GB
     * RAM block so the loader can continue executing after MMU enable.
     */
    ttbr1_pud[1] = (unsigned long long)VIRT_RAM_BASE | PE_KERNEL_CODE | PT_BLOCK_ENTRY;
#endif
...
#if defined(ROS_BOARD_VIRT)
    ttbr0_pud[1] = (unsigned long long)VIRT_RAM_BASE | PE_KERNEL_CODE | PT_BLOCK_ENTRY;
#endif
}
