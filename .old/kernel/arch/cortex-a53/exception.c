...
static int exception_is_user_context(unsigned long spsr) {
  return (spsr & 0xFUL) == PSR_MODE_EL0t;
}

void handle_fatal_user_data_abort(unsigned long esr, unsigned long fault_address, unsigned long elr, unsigned long sp_el0) {
  unsigned long spsr = 0UL;

  asm volatile("mrs %0, spsr_el1" : "=r"(spsr));
  if (!current_task || !current_task->process || (current_task->flags & PF_KTHREAD) || !exception_is_user_context(spsr)) {
    return;
  }

  log_error(
    "fatal user data abort: pid=%ld name=%s esr=0x%lX far=0x%lX elr=0x%lX sp=0x%lX",
    current_task->process->id,
  ...
  printf(
    "[alert] killed user process %ld (%s) after fatal data abort\r\n"
    "        esr=0x%lX far=0x%lX elr=0x%lX sp=0x%lX\r\n",
    current_task->process->id,
    current_task->process->name,
    esr,
    fault_address,
  ...
  terminate_current_process_tree(-1L);
}
