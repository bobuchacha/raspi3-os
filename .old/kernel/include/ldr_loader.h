...
extern "C"
{
    ...
    /* Force-unload module even when outstanding references remain. */
    LDR_RESULT ldr_unload_module_force (PLDR_CONTEXT context, PLDR_MODULE module);

    /* Increase one module's live reference count without remapping it. */
    LDR_RESULT ldr_retain_module (PLDR_CONTEXT context, PLDR_MODULE module);

    /* Drop one live reference and unload when the count reaches zero. */
    LDR_RESULT ldr_release_module (PLDR_CONTEXT context, PLDR_MODULE module);

    /* Resolve one already-loaded module by full path or basename suffix. */
    LDR_RESULT ldr_find_module_by_path (PLDR_CONTEXT context, CONST char *path, PLDR_MODULE *outModule);

    /* Query the current live reference count for diagnostics and policy. */
    Int ldr_module_ref_count (PCLDR_MODULE module);
