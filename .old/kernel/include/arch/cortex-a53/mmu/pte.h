...
#if PAGE_SIZE != 0x1000
#error "Current MMU tables and boot setup are hard-coded for 4KB granules. Update pte.h, early_mmu.c, and linker layout together before changing PAGE_SIZE."
#endif

/* PTE-related */
#define pentry_t unsigned long long // 1 page entry is 8 bytes
...
#define PE_AP_USER_RO ((pentry_t)3 << 6)
#define PE_AP_KERNEL_RW ((pentry_t)0 << 6)
#define PE_AP_USER_RW ((pentry_t)1 << 6)
...
#define PE_KERNEL_DATA ((((pentry_t)MAIR_NON_CACHEABLE_INDEX << 2) | PE_AP_KERNEL_RW | PE_PXN | PE_UXN | PE_ISH) | PE_ACCESSED)
#define PE_KERNEL_RO ((((pentry_t)MAIR_CACHEABLE_INDEX << 2) | PE_AP_KERNEL_RO | PE_PXN | PE_UXN | PE_ISH) | PE_ACCESSED)
#define PE_KERNEL_CODE ((((pentry_t)MAIR_CACHEABLE_INDEX << 2) | PE_AP_KERNEL_RW | PE_UXN | PE_ISH) | PE_ACCESSED)
#define PE_USER_DATA ((((pentry_t)MAIR_NON_CACHEABLE_INDEX << 2) | PE_AP_USER_RW | PE_PXN | PE_UXN | PE_ISH) | PE_ACCESSED)
#define PE_USER_RO ((((pentry_t)MAIR_CACHEABLE_INDEX << 2) | PE_AP_USER_RO | PE_PXN | PE_UXN | PE_ISH) | PE_ACCESSED)
#define PE_USER_CODE ((((pentry_t)MAIR_CACHEABLE_INDEX << 2) | PE_AP_USER_RO | PE_PXN | PE_ISH) | PE_ACCESSED)

#define pte_clear_mair(entry) (*entry &= ~((pentry_t)0x1c))
#define pte_mark_non_cacheable(entry)                      \
    do                                                     \
    {                                                      \
...
#define pte_get_phys(entry) (*entry & ~((((pentry_t)1 << 12) - 1) | ((((pentry_t)1 << 6) - 1) << 53)))
