...
static inline unsigned long ros_syscall4(
    ...
    return invokeSyscall4(number, arg0, arg1, arg2, arg3);
}

static inline unsigned long ros_syscall5(
    unsigned long number,
    unsigned long arg0,
    unsigned long arg1,
    unsigned long arg2,
...
static void ros_format_push_char(FormatSink* sink, char ch) {
    if (!sink) {
        return;
    }
...
static void ros_format_push_text(FormatSink* sink, const char* text) {
    if (!text) {
        text = "(null)";
    }

    while (*text) {
        ros_format_push_char(sink, *text++);
    }
}

static void ros_format_push_padding(FormatSink* sink, int count, char pad) {
    while (count-- > 0) {
        ros_format_push_char(sink, pad);
    }
}

static size_t ros_format_text_length(const char* text) {
    size_t length = 0;
...
static size_t ros_format_unsigned(char* buffer, unsigned long long value, unsigned int base, int uppercase) {
...
static void ros_format_number(
    ...
    char digits[32];
    size_t length = ros_format_unsigned(digits, value, base, uppercase);
    int prefix_length = 0;
    ...
    if (!zero_pad) {
        ros_format_push_padding(sink, width - (int)(length + (size_t)prefix_length), ' ');
    }
    if (negative) {
        ros_format_push_char(sink, '-');
    }
    if (prefix_hex) {
        ros_format_push_char(sink, '0');
        ros_format_push_char(sink, uppercase ? 'X' : 'x');
    }
    if (zero_pad) {
        ros_format_push_padding(sink, width - (int)(length + (size_t)prefix_length), '0');
    }

    for (size_t index = 0; index < length; index++) {
        ros_format_push_char(sink, digits[index]);
    }
}

static int ros_format_vprintf(FormatSink* sink, const char* fmt, va_list args) {
    while (fmt && *fmt) {
        int zero_pad = 0;
        int width = 0;
        int long_count = 0;
        ...
        case '\0':
            break;
        case 'c':
            ros_format_push_char(sink, (char)va_arg(args, int));
            break;
        case 's': {
            const char* text = va_arg(args, const char*);
            size_t length = ros_format_text_length(text);

            ros_format_push_padding(sink, width - (int)length, ' ');
            ros_format_push_text(sink, text);
            break;
        }
        case 'd':
        case 'i': {
            ...
            ros_format_number(sink, magnitude, value < 0, 10, 0, width, zero_pad, 0);
            break;
        }
        case 'u': {
            unsigned long long value;
            ...
            ros_format_number(sink, value, 0, 10, 0, width, zero_pad, 0);
            break;
        }
        case 'x':
        case 'X': {
            ...
            ros_format_number(sink, value, 0, 16, spec == 'X', width, zero_pad, 0);
            break;
        }
        case 'p': {
            unsigned long long value = (unsigned long long)(uintptr_t)va_arg(args, void*);
            ros_format_number(sink, value, 0, 16, 0, width > 0 ? width : 2, 1, 1);
            break;
        }
        default:
            ros_format_push_char(sink, '%');
            ros_format_push_char(sink, spec);
            break;
        }
    }
...
long syscall(long number, ...) {
    ...
    case SYS_UPTIME_MSEC:
        va_end(args);
        return (long)ros_syscall0((unsigned long)number);
    case SYS_WRITE:
    case SYS_MALLOC:
    case SYS_FREE:
    case SYS_EXIT:
    ...
    case SYS_LOG_WRITE:
        ...
        va_end(args);
        return (long)ros_syscall1((unsigned long)number, arg0);
    case SYS_TASK_NAME:
    case SYS_TASK_ARGS:
    ...
    case SYS_LOG_RECV:
        ...
        va_end(args);
        return (long)ros_syscall2((unsigned long)number, arg0, arg1);
    case SYS_SPAWN:
    case SYS_DIR_ENTRY:
        arg0 = va_arg(args, unsigned long);
        arg1 = va_arg(args, unsigned long);
        ...
        va_end(args);
        return (long)ros_syscall3((unsigned long)number, arg0, arg1, arg2);
    case SYS_READ_FILE:
    case SYS_EXT_INVOKE:
        arg0 = va_arg(args, unsigned long);
        arg1 = va_arg(args, unsigned long);
        ...
        va_end(args);
        return (long)ros_syscall4((unsigned long)number, arg0, arg1, arg2, arg3);
    case SYS_MODULE_INVOKE:
        ...
        va_end(args);
        return (long)ros_syscall5((unsigned long)number, arg0, arg1, arg2, arg3, arg4);
    default:
        va_end(args);
        return -1;
    }
...
void call_sys_write(char* buf) {
    (void)syscall(SYS_WRITE, (unsigned long)buf);
}

unsigned long call_sys_malloc(unsigned long value) {
    return (unsigned long)syscall(SYS_MALLOC, value);
}

void call_sys_free(void* ptr) {
    (void)syscall(SYS_FREE, (unsigned long)ptr);
}
...
void* malloc(size_t size) {
    return (void*)call_sys_malloc((unsigned long)(size ? size : 1));
}
...
/*
 * Default executable entry shim.
 *
 * User executables may now provide a plain `int main(void)` and let the shared
 * runtime translate that hosted-style return value into the kernel's `AppMain`
 * entrypoint contract. Programs that still provide their own strong `AppMain`
 * keep overriding this weak fallback unchanged.
 */
int __attribute__((weak)) AppMain(void) {
    int status;

    if (!main) {
        writeLine("missing main/AppMain entry");
        exitProcess(127);
    }
...
int vsnprintf(char* buffer, size_t size, const char* fmt, va_list args) {
    ...
    sink.capacity = size;
    sink.length = 0;
    return ros_format_vprintf(&sink, fmt, args);
}
