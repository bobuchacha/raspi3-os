...
static void shell_command_ls(ShellState* state, const char* arg) {
    ...
    shell_write("\n");
    shell_write("  ");
    shell_write_left("TYPE", SHELL_LS_TYPE_WIDTH);
    shell_write(" ");
    shell_write_left("SIZE", SHELL_LS_SIZE_WIDTH);
    shell_write(" ");
    shell_write_line("NAME");
    shell_write("  ");
    shell_write_left("--------", SHELL_LS_TYPE_WIDTH);
    shell_write(" ");
    shell_write_left("------------", SHELL_LS_SIZE_WIDTH);
    shell_write(" ");
    shell_write_line("------------------------------");
    ...
    while (1) {
        ...
        shell_write("  ");
        shell_write_left((entry.attr & 0x10) ? "dir" : "file", SHELL_LS_TYPE_WIDTH);
        shell_write(" ");
        {
        ...
        shell_write_right(size_text, SHELL_LS_SIZE_WIDTH);
        shell_write(" ");
...
static void shell_command_ipc_test(void) {
    ...
    receiver_pid = shell_spawn_program_nowait("/bin/ipc_demo.exe", "ipc-recv", "recv");
    if (receiver_pid < 0) {
    ...
    sender_pid = shell_spawn_program_nowait("/bin/ipc_demo.exe", "ipc-send", "send ipc-recv");
    if (sender_pid < 0) {
...
static int shell_execute_builtin(ShellState* state, const char* command, const char* arg0, const char* full_args) {
    ...
    if (shell_strcmp(command, "exit") == 0) {
    ...
    if (shell_strcmp(command, "guidemo") == 0) {
    ...
    if (shell_strcmp(command, "ipc-test") == 0) {
